...
const SPONSORED_KEYWORDS = [
  ...
  'Sponsorizzato', 'Patrocinado', 'Gesponsord', 'スポンサー',
...
function parseProducts(html, domain) {
  ...
  for (let i = 1; i < blocks.length; i++) {
    ...
    // Variant/sibling ASINs from color swatch links
    const dpLinks = block.match(/href="\/[^"]*\/dp\/[A-Z0-9]{10}/g) || [];
    const siblingAsins = [];
    dpLinks.forEach(function(link) {
...
function parseCategoryNodes(html) {
  var marker = 'wayfinding-breadcrumbs_feature_div';
  var start = html.indexOf(marker);
  if (start === -1) return [];

  var section = html.substring(start, start + 3000);
